\section{Results}
\paragraph{Energy calibration and gamma spectra?}
...
This was accomplished using the known decay schemes of the four sources, Cesium, Cobalt (which will be studied in more detail later), Lead and Hafnium [METTRE EN BEAU].
These schemes can be found in \cite{notice_generale} [except Hafnium??].
Other sources were attempted (Sodium 22 and Cobalt 60), but their low count made for very long times to get reasonable spectrum.

A gaussian fit was [done] on the visible peaks (those we recognised, let's say).
...
It was then possible to get gamma spectra of the four sources, with energies instead of bins.
