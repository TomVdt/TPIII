...
Using the Doppler effect only yields the relative velocity between the source, the hydrogen cloud, and the observer. However, to obtain the velocity field of the Milky Way, the true velocity is needed. By approximating the orbits of object within the Milky Way to circular orbits, the angular velocity can be found. There exist multiple methods to achieve this. \hl{TODO: merge des phrases}

\paragraph{Tangent-velocity method ($\ell < 90^\circ$)}
TODO

\paragraph{Generalised tangent ($\ell < 90^\circ$)}
TODO
proof in \autoref{sec:proof_inner_circle}

\paragraph{Matteo method\texttrademark\copyright \ ($90^\circ < \ell < 270^\circ$)}
TODO
