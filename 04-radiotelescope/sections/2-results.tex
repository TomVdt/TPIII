...
\autoref{fig:VEGA_velocity_curve} depicts the velocity curves obtained for each method. For $R < R_0$, the velocity seems to grow linearly up to around $V=180$ \si{\kilo\meter\per\second}. In the region $R > R_0$, multiple behaviors are observed: one branch remains approximately constant at around $240$ \si{\kilo\meter\per\second}, while another branch grows to above \mbox{$500$ \si{\kilo\meter\per\second}}
...
\begin{figure}[htbp]
    ...
    \begin{minipage}[t]{0.5\textwidth}
        ...
        \caption{Position of the hydrogen clouds observed by VEGA. Obtained for $\ell < 90^\circ$ using \autoref{eq:general_radius}. Only showing the positive solution for the distance $d_+$}
